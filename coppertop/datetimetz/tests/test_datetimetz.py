...
from .._core import UTC, Date, TimeTz, DateTimeTz, YMDHMS, HMS, YearMonth, MonthDay, Precision, \
    ParseUTC, ParseDate, ParseTimeTz, ParseDateTimeTz, ParseTz, ParseYMDHMS, ParseHMS, ParseYearMonth, \
    ToUTC, ToTz, ToString, \
    Tz, FpMLCity, IanaCity, IanaTz, \
    YYYY_MM_DD
...
def test_parsing():
    ...
    # UTC
    "2020.01.01 16:15" >> ParseUTC(format='yyyy.MM.dd hh:mm') >> AssertEqual >> UTC(2020,1,1,16,15)


    # Date
    "2020.01.01" >> ParseDate(format=YYYY_MM_DD) >> AssertEqual >> Date(2020, 1, 1)


    # ParseDateTimeTz
    "2020.01.01 16:15:0.001 GBLO" >> ParseDateTimeTz(format='yyyy.MM.dd HH:MM:SS.us FFFF') >> AssertEqual >> DateTimeTz(2020,1,1,16,15,0,1,Precision.s)
    ...
    ymdhms >> ToTz(FpMLCity.GBLO) >> AssertEqual >> DateTimeTz(
        "2020.06.01" >> ParseDate(format='yyyy.MM.dd'),
        "16:15:00.0001" >> ParseHMS(format='HH:MM.SS.us'),
        FpMLCity.GBLO
    )
    ...
    "H0" >> ParseYearMonth(format='HMUZy', observationDate=Date(2020,4,1)) >> AssertEqual >> YearMonth(2030,3)
...
def test_formatting():
    ...
    UTC(2020, 1, 1, 16, 15) >> ToString('yyyy.MM.dd HH:MM UTC') >> AssertEqual >> 'UTC(2020, 1, 1, 16, 15)'
    Date(2020, 1, 1) >> ToString('') >> AssertEqual >> 'Date(2020, 1, 1)'
